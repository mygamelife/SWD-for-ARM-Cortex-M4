...
void test_tlvSendService_should_send_if_tx_is_ready(void)
{
  uartInit_Ignore();
	Tlv_Session *session = tlvCreateSession();
  uint8_t buffer[] = {0xFE, 0xCA, 0xEF, 0xBE};
...
void test_tlvReceiveService_should_receive_type_first_then_follow_by_length_and_data(void)
{
  uartInit_Ignore();
  Tlv_Session *session = tlvCreateSession();
  ...
  /* Received Type */
  getByte_ExpectAndReturn(session->handler, &session->rxBuffer[0], UART_OK);
  /* Received Length */
  getByte_ExpectAndReturn(session->handler, &session->rxBuffer[1], UART_OK);
  /* Received Data */
  getBytes_ExpectAndReturn(session->handler, &session->rxBuffer[2], 5, UART_OK);
  getSystemTime_ExpectAndReturn(10);
  isRxBusy_ExpectAndReturn(1);
...
void test_tlvReceiveService_should_yield_if_type_is_not_arrive(void)
{
  uartInit_Ignore();
  Tlv_Session *session = tlvCreateSession();
  ...
  getByte_ExpectAndReturn(session->handler, &session->rxBuffer[0], UART_BUSY);
  tlvReceiveService(session);
...
void test_tlvReceiveService_should_set_time_out_flag_if_no_data_arrive_after_first_2_byte(void)
{
  ...
  Try {
    ...
    getBytes_ExpectAndReturn(session->handler, &session->rxBuffer[2], 5, UART_OK);
    getSystemTime_ExpectAndReturn(10);
    isRxBusy_ExpectAndReturn(0);
    isTimeout_ExpectAndReturn(ONE_SECOND, 10, 1); //timeout
    cancelRx_Expect();

    tlvReceiveService(session);
    printf("Should Throw TLV_TIMEOUT\n");
  }
  Catch(err) {
    TEST_ASSERT_EQUAL(TLV_TIMEOUT, err);
    TEST_ASSERT_EQUAL(0, isYielding);
    TEST_ASSERT_EQUAL(FLAG_CLEAR, GET_FLAG_STATUS(session, TLV_DATA_RECEIVE_FLAG));
  }
}
...
void test_tlvService_should_able_to_receive_while_sending(void)
{
  ...
  /* Send Packet */
  isTxBusy_ExpectAndReturn(1);
  sendBytes_ExpectAndReturn(session->handler, session->txBuffer, tlv->length + 2, UART_OK);
...
void test_verifyTlvPacket_should_throw_error_if_nack(void)
{
  ...
  Try {
    ...
    Tlv *tlv = tlvCreatePacket(TLV_NOT_OK, 1, &errorCode);
    verifyTlvPacket(tlv);
    printf("Should Throw TLV_TIMEOUT Exception!");
  }
  Catch(err)  {
    TEST_ASSERT_EQUAL(TLV_TIMEOUT, err);
  }
