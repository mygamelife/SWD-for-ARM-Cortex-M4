...
void test_tlvPackPacketIntoTxBuffer_should_get_data_from_elf_file_then_pack_into_txBuffer(void)
{
  ...
  TLV_TypeDef *tlv = tlvCreateNewPacket(TLV_WRITE);
  result = tlvPutDataIntoBuffer(tlv, pElfSection);
  ...
  chksum = tlvCalculateCheckSum(tlv->value, tlv->length, ADDRESS_START_POSITION);
  TEST_ASSERT_EQUAL(txBuffer[tlv->length - 1], chksum);
...
void test_tlvDecodePacket_should_decode_after_receive_TLV_Packet(void)
{
  ...
  /* Decode txBuffer */
  TLV_TypeDef *tlvDecode = tlvDecodePacket(txBuffer);
