...
{ 
  ...
  sendBytes_ExpectAndReturn(session.handler, session.txBuffer, tlv->length + 2, HAL_OK);
  tlvSendService(&session);
  TEST_ASSERT_EQUAL(END_SEND, session.sendState);
}
...
void test_tlvReceiveService_rxBuffer_should_stored_tlv_packet(void)
{
  ...
  getBytes_ExpectAndReturn(session.handler, session.rxBuffer, 2, HAL_OK); //data arrive
  getBytes_ExpectAndReturn(session.handler, &session.rxBuffer[2], 5, HAL_OK);
...
void test_tlvReceiveService_rxBuffer_should_stored_null_if_no_data_arrive(void)
{
  ...
  getBytes_ExpectAndReturn(session.handler, session.rxBuffer, 2, HAL_ERROR); //data arrive
...
void test_tlvReceiveService_should_set_time_out_flag_when_data_didnt_arrive_after_first_2_byte(void)
{
  ...
  getBytes_ExpectAndReturn(session.handler, session.rxBuffer, 2, HAL_OK); //data arrive
  getBytes_ExpectAndReturn(session.handler, &session.rxBuffer[2], 10, HAL_ERROR); //timeout occur
...
void test_tlvService_should_able_to_receive_while_sending(void)
{
  ...
  sendBytes_ExpectAndReturn(session.handler, session.txBuffer, tlv->length + 2, HAL_OK);
  getBytes_ExpectAndReturn(session.handler, session.rxBuffer, 2, HAL_OK);
  getBytes_ExpectAndReturn(session.handler, &session.rxBuffer[2], 5, HAL_OK);
  
  tlvService(&session);
  
  TEST_ASSERT_EQUAL(END_SEND, session.sendState);
  TEST_ASSERT_EQUAL(TRANSMISSION_FREE, session.tState);
  TEST_ASSERT_EQUAL(true, session.DATA_ARRIVE_FLAG);
